...
def monitor_tool(request: ToolCallRequest, 
    ...
    try:
        ...
        logger.error(f"[toll monitor] 调用{request.tool_call['name']}失败：{str(e)}")
        raise e
...
def log_before_model(
    ...
    '''
    # 这里的 state['messages'] 已经是正确的写法（外部双引内部单引）
